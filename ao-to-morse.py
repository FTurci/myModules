...
def second_virial(potential,params, beta,rmax=20.0, npoints=10000):
	...
	I = simps(y,r)*2*pl.pi #don't forget the 2pi!
	return I
...
reduced_B2s_morse = []
epsilons = pl.linspace(min(Uao)/2.,max(Uao)*2,10000)
beta = 1.0
...
for ao_p in ao_params:
	...
	# pl.plot(epsilons, reduced_B2_ao*np.ones(len(epsilons)), '-k')
	# pl.plot(epsilons, reduced_B2s_morse, '-')
	# pl.show()	
